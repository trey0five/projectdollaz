...
4. **Catholic/diocesan is the wedge.** It's where finance + multi-school consolidation pain is sharpest and where the scan found the consolidation white space largest.
...
|---|---|---|
| **Financial (vertical)** | **~80%.** Data hub (sources incl. **QBO** connector), mapping/normalisation, Statements (validation + reconciliation + reporting), Budget (+ forecast, budget-vs-actual, **diocesan roll-up**), **Monthly actuals**, Reports + **Board packets**, period history | deepen QBO sync; promote diocesan consolidation from "budget roll-up" to full financials as a headline |
...
| **Org / diocesan** | Organizations module + DioceseRollup (budget) | full multi-school financial consolidation + scope-as-boundary roll-ups |
...
### Phase 0 — Finish the financial wedge + diocesan consolidation *(in progress)*
...
- **Scope.**
  ...
  - Promote **diocesan consolidation** from budget roll-up to **full consolidated financials** across schools (the DioceseRollup pattern, extended to statements + cash).
  - Close audit-trail gaps so every figure traces to source (this is also a Phase-1 prerequisite).
- **Unlocks.** A diocese connects N schools' QBO → normalised, trusted statements per school + a consolidated diocesan view + board packets.
- **Dependencies.** None (foundational).
- **Definition of done.** A design-partner diocese runs month-end on the platform: per-school statements + consolidated diocesan financials + a board packet, every number traceable, with no spreadsheet step.
...
- **Scope (delta over what exists).**
  1. **Promote the metric registry → a canonical semantic layer.** Formalise `MetricMeta` so each figure is defined *once*: formula, inputs, owning domain, **scope-aggregation rule** (how it rolls up school→diocese→system), thresholds (reuse `health.js` bands), and **lineage to source**. Everything — modules, reports, briefing — reads from here.
...
- **Dependencies.** Phase 1 semantic layer + briefing; ≥1 SIS connector (pick the one your design-partner diocese uses).
...
- **Thesis.** Board packets already exist; dioceses and boards feel governance pain acutely; and governance *requires* the first real workflow primitives — so these ship together.
- **Scope.**
...
- **Unlocks.** Search finds anything; accreditation evidence accrues from daily operations (a major Catholic/independent-school pain); all eight domains feed ≥1 briefing indicator — the full digital COO.
...
| **Solution packs** | Config over one architecture: org hierarchy, compliance rules, templates, report packages. **Catholic/diocesan first**, then independent → charter → private network → small college. | Catholic from Phase 0; others as demand pulls |
| **Trust, RBAC & audit** | Scope-as-boundary (you see only your entitlement), full audit trail, security posture (SOC 2 track) as you reach diocese scale. | continuous |
| **Positioning / GTM** | Lead with the two unclaimed things — **the briefing** and **diocesan consolidation** — finance as proof. Land a design-partner diocese early; expand school-by-school. | continuous |
...
|---|---|---|---|
| **0** | Finish the wedge | Trusted per-school + **consolidated diocesan** financials, board packets | a diocese runs month-end with no spreadsheet step |
...
- **Diocesan consolidation as a constant headline:** the scan found it "largely unclaimed" by the SIS/suite incumbents (the tool that surfaces is ParishSOFT, a separate parish vendor) — so it's both a wedge and a differentiator, not a phase.
...
2. **First design-partner diocese** — anchors Phase 0–1 DoD and the first SIS to support (Phase 2).
3. **Build-vs-integrate per non-finance domain** — confirm enrollment-first, and which SIS / HRIS / donor CRM to connect first.
4. **How hard to push solution packs beyond Catholic/diocesan** before the briefing is proven (recommendation: don't — prove the wedge, then templatise).
