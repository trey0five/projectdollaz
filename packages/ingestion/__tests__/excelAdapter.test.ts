...
// assert against. The sample-data .xlsx files are generated by
// scripts/gen-sample-data.mjs, which writes these 26 account rows AND the
// engine fixture together — so they must parse back byte-identical. If a future
// sample-data edit changes any balance, this fails loudly at the ingestion
// layer instead of silently diverging from
// packages/engine/__tests__/__fixtures__/sampleRows.json.
...
describe('excel adapter', () => {
  it('parses the current-year sample into normalized rows', () => {
    ...
    // Tuition (revenue) parses as a credit (negative); salaries as a debit.
    expect(rows.find((r) => r.acct === 401)!.total).toBeLessThan(0)
    expect(rows.find((r) => r.acct === 500)!.total).toBeGreaterThan(0)
  })
