...
describe('student_teacher_ratio — registry wiring', () => {
  it('is registered after enrollment_change_yoy, first 12 keys byte-identical', () => {
    ...
    // enrollment_vs_plan (Phase 2) + the six Phase-6 hr/planning keys were
    // appended AFTER student_teacher_ratio, so STR is now the eighth-to-last key.
    expect(METRIC_KEYS[METRIC_KEYS.length - 8]).toBe('student_teacher_ratio')
