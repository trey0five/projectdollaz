...
export default function IntakeGuide() {
  return (
    <div className="rounded-2xl border border-rule bg-white p-4 text-left">
      <p className="mb-3 flex items-center gap-1.5 text-[12px] font-semibold uppercase tracking-[0.1em] text-navy">
        ...
        {SLOT_ROLES.map((role) => {
          ...
          return (
            <div key={role} className="flex gap-2.5">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-gold-gradient text-[12px] font-bold text-navy">
                {m.step}
              </span>
              <div className="min-w-0">
                <p className="text-[13px] font-semibold text-navy">
                  {m.plainLabel}
                  <span
                    className={`ml-1.5 align-middle text-[10px] font-semibold uppercase tracking-[0.06em] ${
                ...
                <p className="mt-0.5 text-[12px] leading-snug text-muted">{m.blurb}</p>
                <p className="mt-0.5 text-[11px] italic text-muted">{m.source}</p>
              </div>
            </div>
          )
        })}
      ...
      <p className="mt-3 flex items-start gap-1.5 rounded-lg bg-section px-3 py-2 text-[12px] text-ink">
