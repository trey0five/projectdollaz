...
export default function AutosaveIndicator({ saving, dirty, error }) {
  ...
  if (saving) {
    return (
      <span className="inline-flex items-center gap-1.5 text-[12px] font-medium text-navy">
    ...
    <div className={`flex flex-wrap items-center justify-between gap-3 ${className}`}>
      ...
      {dirty && !saving && (
        <button
          type="button"
          onClick={onSaveNow}
          className="text-[12px] font-semibold uppercase tracking-[0.06em] text-gold transition-colors hover:text-gold-light"
        >
          Save now
        </button>
      )}
